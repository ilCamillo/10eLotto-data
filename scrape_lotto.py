...
def scrape():
    ...
    try:
        ...
        if not numeri_divs: 
            print("Numeri non trovati")
            return None
        numeri = " ".join([n.text.strip() for n in numeri_divs[:20]])
        
        # Oro e Doppio Oro
        oro_tag = soup.find('div', class_='num_estratto_oro')
        doro_tag = soup.find('div', class_='num_estratto_doppio_oro')
...
def update_csv(new_line):
    ...
    # Verifica se l'estrazione è già presente (confronto pulito)
    if any(new_line in l for l in lines):
        print("Dati già presenti nel CSV.")
        return
